...
test("固定收支與財務分析為獨立頁面", async () => {
  ...
  for (const text of ["本月財務異常", "本月消費趨勢", "近 1 個月比較", "支出分析", "本月盈虧", "儲蓄率", "固定與可變支出", "預估月底總支出"]) assert.match(analysisHtml, new RegExp(text));
  assert.doesNotMatch(analysisHtml, /所有支出與收入|每月固定收入/);
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  assert.match(page, /donutBackground/);
  assert.match(page, /monthDays/);
  assert.match(page, /transaction\.date === key/);
  assert.match(page, /本月尚無支出資料/);
  ...
  assert.match(page, /近 1 個月比較/);
  assert.doesNotMatch(page, /近 6 個月比較|最近 7 天/);
