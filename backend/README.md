...
FastAPI 後端，支援 SQL Server 或 SQLite，提供交易、固定扣款、銀行 CSV 與密碼保護 PDF 去重 API。
...
前端若要使用 PDF 匯入，請設定 `NEXT_PUBLIC_SELFBANK_API_URL=http://localhost:8000` 後重新建置。PDF 密碼是每次請求的必填欄位，只在記憶體中解密該次檔案，不會保存到設定檔、資料庫或日誌。目前支援台北富邦「臺幣活期存款交易明細」表格，以及每列為 `YYYY/MM/DD 名稱 金額` 的通用文字 PDF；掃描影像及其他銀行版型需另加 OCR 或專用解析規則。
